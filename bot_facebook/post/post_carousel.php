<?php
// បង្កើន Time Limit ព្រោះការ Upload វីដេអូអាចប្រើពេលយូរ
set_time_limit(3600); // 1 ម៉ោង

// Function សម្រាប់បង្ហាញសារទៅ User
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ពិនិត្យមើលថាមាន File Upload ដែរឬទេ
    if (!isset($_FILES['videos']) || !is_array($_FILES['videos']['name']) || empty($_FILES['videos']['name'][0])) {
        show_response("សូមជ្រើសរើសវីដេអូយ៉ាងហោចណាស់មួយ។", true);
    }
    
    $video_ids = [];
    $total_files = count($_FILES['videos']['name']);

    // --- ជំហានទី១: Upload វីដេអូនីមួយៗដើម្បីទទួលបាន Video ID ---
    for ($i = 0; $i < $total_files; $i++) {
        // ពិនិត្យមើល Error ពេល Upload
        ...
        $post_data_upload = [
            ...
            // ប្រើ CURLFile សម្រាប់បញ្ជូន File
        ...
        if ($http_code == 200 && isset($result['id'])) {
            $video_ids[] = $result['id']; // ប្រមូល ID ដែលបាន Upload ជោគជ័យ
        } else {
            $error_message = isset($result['error']['message']) ? $result['error']['message'] : 'Unknown error during video upload.';
            show_response("បរាជ័យក្នុងការ Upload វីដេអូ '{$file_name}': " . htmlspecialchars($error_message), true);
        }
    }

    // --- ជំហានទី២: បង្កើត Carousel Post ដោយប្រើ Video IDs ដែលប្រមូលបាន ---
    if (!empty($video_ids)) {
        $child_attachments = [];
        foreach ($video_ids as $vid_id) {
            $child_attachments[] = [
                'video_id' => $vid_id,
                'link' => "https://www.facebook.com/{$page_id}/", // Link ទៅកាន់ Page ជា Default
                'name' => 'ទស្សនាវីដេអូថ្មី!', // ចំណងជើង Card ជា Default
            ];
        }
        ...
        if ($http_code == 200 && isset($result['id'])) {
            ...
            show_response("បរាជ័យក្នុងការបង្កើត Carousel Post: " . htmlspecialchars($error_message), true);
        }
    } else {
        show_response("គ្មានវីដេអូណាមួយត្រូវបាន Upload ជោគជ័យទេ។ មិនអាចបង្កើត Post បានទេ។", true);
    }
}
?>