...
try {
    ...
    error_log('table.php - user_id: ' . ($_SESSION['user_id'] ?? 'not set') . ', is_admin: ' . ($is_admin ? 'true' : 'false'));
...
if (isset($_GET['delete_id']) && isset($_GET['csrf_token'])) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            error_log('table.php - Employee deleted, id: ' . $delete_id . ', user_id: ' . $_SESSION['user_id']);
        } else {
            $_SESSION['error'] = 'រកមិនឃើញបុគ្គលិកដែលត្រូវលុប!';
            error_log('table.php - No employee found for deletion, id: ' . $delete_id);
        }
    } catch (PDOException $e) {
...
try {
    ...
    error_log('table.php - Query executed, rows fetched: ' . count($employees));
} catch (PDOException $e) {
                        ...
                        <?php foreach ($employees as $employee): ?>
                            <tr>
                                ...
                                <td data-label="សកម្មភាព">
                                    <a href="form_input.php?employee_id=<?php echo $employee['id']; ?>" class="action-btn edit-btn">កែប្រែ</a>
                                    <a href="table.php?delete_id=<?php echo $employee['id']; ?>&csrf_token=<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>" class="action-btn delete-btn" onclick="return confirm('តើអ្នកប្រាកដជាចង់លុបបុគ្គលិកនេះមែនទេ?');">លុប</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
