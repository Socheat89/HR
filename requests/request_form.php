...
<body>
    <div class="container-card">
        <div class="form-header">
            ...
            <h2>сссс╛сс╗ссссс╖с/ссссс╢сс</h2>
        </div>
        
        <button class="btn btn-blue btn-back">
            <i class="fas fa-arrow-left"></i> сРсссссс
        </button>
        ...
        <form action="../requests/submits_request.php" method="POST" accept-charset="UTF-8" onsubmit="return validateForm()">
            <!-- Main Details Section -->
            <div class="form-grid">
                <div class="form-group">
                    <label for="number">ссБсБссс (Reference No) <span>*</span></label>
                    <input type="text" name="number" id="number" value="<?= htmlspecialchars($request['number'] ?? '') ?>" placeholder="сз. VVC-2024-001" required>
                </div>
                <div class="form-group">
                    <label for="request_date">сс╢сссс╖ссссБссссс╛сс╗с <span>*</span></label>
                ...
                <div class="form-group">
                    <label for="request_person">ссссссвссссссс╛сс╗с <span>*</span></label>
                    <input type="text" name="request_person" id="request_person" value="<?= htmlspecialchars($request['request_person'] ?? '') ?>" placeholder="ссссБсссссс" required>
                </div>
                <div class="form-group">
                    <label for="position">сПс╜сс╢сс╕</label>
                    <input type="text" name="position" id="position" value="<?= htmlspecialchars($request['position'] ?? '') ?>" placeholder="ссссБссПс╜сс╢сс╕">
                </div>
                <div class="form-group">
                    <label for="department">ссссс</label>
                    <input type="text" name="department" id="department" value="<?= htmlspecialchars($request['department'] ?? '') ?>" placeholder="ссссБссссссссссс">
                </div>
                <div class="form-group">
                    <label for="project">сссссс</label>
                    <input type="text" name="project" id="project" value="<?= htmlspecialchars($request['project'] ?? '') ?>" placeholder="ссссБсссссссссссс">
                </div>
                <div class="form-group">
                    <label for="none_date">сс╢сссс╖ссссБссПссс╝ссс╢с</label>
                    <input type="date" name="none_date" id="none_date" value="<?= htmlspecialchars($request['none_date'] ?? '') ?>">
                </div>
                <div class="form-group">
                    <label for="advance_type">сссссБссссс╢ссссс <span>*</span></label>
                    <select name="advance_type" id="advance_type" required>
                        <option value="" <?= empty($request['advance_type']) ? 'selected' : '' ?>>-- сс╝ссссс╛ссс╛с --</option>
                        <option value="Project Advance" <?= ($request['advance_type'] ?? '') === 'Project Advance' ? 'selected' : '' ?>>сссс╢сссс╗ссБссссс╢сссссс╢ссссссс</option>
                        <option value="Personal Advance" <?= ($request['advance_type'] ?? '') === 'Personal Advance' ? 'selected' : '' ?>>сссс╢сссс╗ссБссссс╢ссссс╢сссБссс╜с</option>
                        <option value="Mission Advance" <?= ($request['advance_type'] ?? '') === 'Mission Advance' ? 'selected' : '' ?>>сссс╢сссс╗ссБссссс╢сссБсссссс</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="deadline">сс╢сссс╖ссссБссс╝сс╢сПс</label>
                    <input type="date" name="deadline" id="deadline" value="<?= htmlspecialchars($request['deadline'] ?? '') ?>">
                </div>
                <div class="form-group md-col-span-2">
                    <label for="in_words">ссс╗ссс╢свсссс <span>*</span></label>
                    <input type="text" name="in_words" id="in_words" value="<?= htmlspecialchars($request['in_words'] ?? '') ?>" placeholder="сз. сс╜ссссс╗сссс╢сссПс" required>
                </div>
            </div>
                    ...
                    <table id="itemsTable">
                        <thead>
                            <tr>
                                ...
                                <th>сПсссссс╢с</th>
                                <th>сПссссссс╗с</th>
                                <th class="action-cell">ссссссс╢с</th>
                            </tr>
                        </thead>
                        <tbody>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="4">ссс╗ссс╣ссссс╢сс:</td>
            ...
            <button type="submit" class="btn btn-gold submit-btn">
                <i class="fa fa-save"></i> ссссс╝ссссс╛
            </button>
        </form>
    </div>
    ...
    <nav class="bottom-nav">
        ...
        <a href="../system/checklist.php" class="nav-item <?= $current_page === '../system/checklist.php' ? 'active' : '' ?>">
    ...
    <script>
        function addItemRow() {
            ...
            row.innerHTML = `
                <td>${index + 1}</td>
                <td><input type="text" name="items[${index}][item_name]" class="item-name" placeholder="ссссссссс╖с" required></td>
        ...
        function confirmDeleteItem(button) {
            if (confirm('сПс╛свссссс╖сПсс╢ссссс╗ср╕гр╕▓р╕вр╕Бр╕▓р╕гссБссссссБ?')) {
                const table = document.getElementById('itemsTable').getElementsByTagName('tbody')[0];
                if (table.rows.length > 1) {
                    ...
                    alert('сПссс╝ссс╢ср╕гр╕▓р╕вр╕Бр╕▓р╕гссс╢ссасссс╢сссс╜сс');
                }
            }
        }
        ...
        function validateForm() {
            const rows = document.getElementById('itemsTable').getElementsByTagName('tbody')[0].rows;
            if (rows.length === 0) {
                 alert('сс╝сссссРсср╕гр╕▓р╕вр╕Бр╕▓р╕гсссс╖сссс╢ссасссс╢сссс╜сс');
                 return false;
            }
            for (let row of rows) {
                const itemName = row.querySelector('.item-name').value.trim();
                if (!itemName) {
                    alert('сс╝сссссБсссссссссс╖сссссс╢ссссссср╕гр╕▓р╕вр╕Бр╕▓р╕гс');
                    return false;
                }
            }
            return true;
