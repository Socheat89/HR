...
define('DB_PASSWORD', 'attendance@2025'); // <-- ត្រូវប្តូរទៅជាពាក្យសម្ងាត់ពិតប្រាកដ
define('DB_NAME', 'samann1_attendance_db');
define('TELEGRAM_BOT_TOKEN', '7680086124:AAHrvdz-mOx3pO1Ijqvh7BHTeGh2JB5JuwQ'); // <-- ត្រូវប្តូរ
define('TELEGRAM_CHAT_ID', '-1002802610249');       // <-- ត្រូវប្តូរ
            ...
            if ($stmt_insert->execute()) {
                ...
                $status_icon = ($status_for_db === 'Good') ? $icon_good : $icon_late;
                    ...
                    if (strcasecmp($status_for_db, 'Late') === 0) {
                        ...
                        if ($stmt_pv = $mysqli->prepare("SELECT start_time FROM attendance_rules WHERE employee_id = ? AND type = ? AND status = 'Good' AND end_time <= ? ORDER BY end_time DESC LIMIT 1")) {
                            $stmt_pv->bind_param('sss', $employee_id, $type, $current_hms);
                            if ($stmt_pv->execute()) {
                                $res_pv = $stmt_pv->get_result();
                                if ($row_pv = $res_pv->fetch_assoc()) {
                                    $pivot_time = $row_pv['start_time'];
                                }
                            }
                            $stmt_pv->close();
                        }
...
<head>
    ...
    <!-- Local PWA manifest (served via PHP to avoid BOM/whitespace issues) -->
    ...
    <style>
        ...
        @font-face {
            ...
            src: url('font/MomoTrustDisplay-Regular.woff2') format('woff2'),
                 url('font/MomoTrustDisplay-Regular.woff') format('woff'),
                 url('font/MomoTrustDisplay-Regular.ttf') format('truetype');
            font-weight: 400;
            font-style: normal;
            font-display: swap;
...
<div id="pwa-install-prompt" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.55); backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); z-index:5000; align-items:center; justify-content:center;">
               ...
               <?php else: ?>
                <div id="card-menu-view" class="main-view active">
                    ...
                    <div class="card-menu">
                        ...
                        <div class="menu-card" onclick="window.location.href='view_logs.php'">
    ...
    <footer class="app-footer" role="navigation" aria-label="Footer navigation">
        ...
        <div class="footer-nav" role="menu" aria-label="Main footer actions">
            ...
            <?php if (get_setting_typed('show_view_logs_card', '1') == '1'): ?>
            <a role="menuitem" class="footer-btn" href="view_logs.php" aria-label="កំណត់ត្រា">
