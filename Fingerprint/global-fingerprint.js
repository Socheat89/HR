...
async function showScanSection(user) {
    ...
    // Logic ថ្មី៖ ហៅ Function ដើម្បីកំណត់ប្រភេទស្កេនដោយស្វ័យប្រវត្តិ
    showStatus(elements.statusEl, 'កំពុងកំណត់ប្រភេទស្កេន...');
    const nextAction = await determineNextScanAction(user);
    elements.actionSelect.value = nextAction;
    showStatus(elements.statusEl, `បានកំណត់ទៅ ${nextAction} ដោយស្វ័យប្រវត្តិ`, 'success');
...
// This `submitScan` function is now a single, authoritative version.
async function submitScan(formData, date, time, location, address, scanStatus, lateMinutes = 0) {
    ...
    if (userId && scanType) {
        setLastScanTypeToStorage(userId, scanType);
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            // បញ្ជូនទៅ Telegram នៅផ្ទៃខាងក្រោយ (fire-and-forget), មិនបាច់រង់ចាំ
            sendToTelegram(telegramMessage).catch(error => {
                console.error('Failed to send to Telegram:', error);
                // បង្ហាញ Error តែមិន Block user
            ...
            // កំណត់ប្រភេទស្កេនបន្ទាប់ដោយស្វ័យប្រវត្តិ ក្រោយពីស្កេនបានជោគជ័យ
            const user = await getFromDB('loggedInUser', 'user');
            const nextAction = await determineNextScanAction(user);
            elements.actionSelect.value = nextAction;
            
            setScanCooldown(2); // កំណត់ Cooldown 2 នាទីក្រោយស្កេនជោគជ័យ
