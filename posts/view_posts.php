...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: ../posts/view_posts.php?lang=$lang&category=" . (isset($_GET['category']) ? intval($_GET['category']) : 0) . "&page=" . (isset($_GET['page']) ? intval($_GET['page']) : 1));
    exit();
}
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <hr>
                    <small><strong>בבבבבב (English):</strong><br><?= nl2br(htmlspecialchars($row['content_en'])) ?></small>
                    <?php endif; ?>
                </td>
                <td><?= $row['created_at'] ?></td>
                <td>
                    <a href="../posts/edit_post.php?id=<?= $row['id'] ?>&lang=<?= $lang ?>">בבבבבב / Edit</a>
                </td>
            </tr>
            <?php endwhile; else: ?>
                <tr><td colspan="6" style="text-align:center;">בבבב¶בבב·בבבבבב</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</body>
