...
if (!isset($_SESSION['user_id'])) {
    $_SESSION['error'] = 'сс╝ссс╝ссссс╕сс╖с!';
...
try {
    ...
    $_SESSION['error'] = 'сссас╗ссссссБсссБс! сс╝ссс╢сссссвссссссссссссс';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create' && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    if (!isset($_POST['title'], $_POST['date'], $_POST['text'], $_POST['user_ids'])) {
        $_SESSION['error'] = 'сс╝сссссБсссссссс╢ссс╢сссвсс!';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
    $user_ids = array_filter((array)$_POST['user_ids'], fn($id) => is_numeric($id) && $id > 0);
    if (empty($user_ids)) {
        $_SESSION['error'] = 'сс╝ссссс╛ссс╛ссс╗ссссс╖сссс╢ссПс╖ссссс╢сс!';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
    $db->beginTransaction();
    try {
        ...
        $user_list = $user_names ? "\nЁе <b>свсссссс╜с</b>: " . implode(', ', $user_names) : '';
        
        $message = "Ёв <b>сс╢ссс╝ссссс╣ссРссс╕: $title</b>\n"
                 . "Ё <i>$date</i>\n"
                 . "ЁЭ <b>сБссс╣ссс╢с</b>:\n$text\n"
                 . $user_list;
        ...
        $inlineKeyboard = [
            [
                ['text' => 'сс╛сссссвс╖сП', 'url' =>'https://app.vvc.asia/announcements.php?id=' . $announcement_id],
        ...
        if (sendTelegramMessage($telegramBotToken, $telegramChatId, $telegramThreadId, $message, $inlineKeyboard)) {
            $_SESSION['success'] = 'сс╢ссс╝ссссс╣ссс╢сссссасс сс╖ссссс╛сс Telegram ссссссссРс!';
        } else {
            $_SESSION['error'] = 'сс╢ссс╝ссссс╣ссс╢сссссасс ссс╗сссПссс╢сссссас╢сссс╗ссс╢ссссс╛сс Telegram!';
        }
        ...
        $_SESSION['error'] = 'сссас╗ссссс╗ссс╢сссссасс! сс╝ссссс╢сс╢ссссПссссПс';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit' && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    ...
    try {
        ...
        $message = "Ёв <b>сс╢ссс╝ссссс╣ссс╢ссссссс: $title</b>\n"
                 . "Ё <i>$date</i>\n"
                 . "ЁЭ <b>сБссс╣ссс╢с</b>:\n$text\n"
                 . $user_list;
        $message = mb_substr($message, 0, 4096, 'UTF-8');
        ...
        $inlineKeyboard = [
            [
                ['text' => 'сс╛сссссвс╖сП', 'url' =>'https://app.vvc.asia/announcements.php?id=' . $_POST['announcement_id']],
        ...
        // Send to Telegram
        if (sendTelegramMessage($telegramBotToken, $telegramChatId, $telegramThreadId, $message, $inlineKeyboard)) {
            $_SESSION['success'] = 'сс╢ссс╝ссссс╣ссс╢ссссссс сс╖ссссс╛сс Telegram ссссссссРс!';
        } else {
            $_SESSION['error'] = 'сс╢ссс╝ссссс╣ссс╢ссссссс ссс╗сссПссс╢сссссас╢сссс╗ссс╢ссссс╛сс Telegram!';
        }
        ...
        $_SESSION['error'] = 'сссас╗ссссс╗ссс╢ссссссс! сс╝ссссс╢сс╢ссссПссссПс';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete']) && hash_equals($_SESSION['csrf_token'], $_GET['csrf_token'])) {
    ...
    try {
        ...
        // Prepare Telegram message
        $message = "Ё <b>сс╢ссс╝ссссс╣ссс╢ссс╗с</b>\n"
                 . "Ёв <b>сссссс╛с</b>: $title\n"
                 . "Ё ID: {$_GET['delete']}";
        $message = mb_substr($message, 0, 4096, 'UTF-8');

        // Send to Telegram
        if (!sendTelegramMessage($telegramBotToken, $telegramChatId, $telegramThreadId, $message)) {
            $_SESSION['error'] = 'сс╢ссс╝ссссс╣ссс╢ссс╗с ссс╗сссПссс╢сссссас╢сссс╗ссс╢ссссс╛сс Telegram!';
        } else {
            $_SESSION['success'] = 'сс╢ссс╝ссссс╣ссс╢ссс╗сссссссссРс!';
        }
        ...
        $_SESSION['error'] = 'сссас╗ссссс╗ссс╢ссс╗с! сс╝ссссс╢сс╢ссссПссссПс';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
}
...
try {
    ...
    $_SESSION['error'] = 'сссас╗ссссс╗ссс╢ссс╢ссс╖сссссРс! сс╝ссссс╢сс╢ссссПссссПс';
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}
?>
...
<head>
    ...
    <title>ссссасссс╢ссс╝ссссс╣с | HRM Dashboard</title>
...
<body>
    <div class="app-container">
        ...
        <header class="app-header animate__animated animate__fadeIn">
            <a href="../homes.php">
                <div class="logo-container">
                    ...
                    <h1 class="app-title">ссссасссс╢ссс╝ссссс╣с</h1>
                </div>
            </a>
            <div class="user-profile">
                ...
                <a href="?logout=true&csrf_token=<?php echo $_SESSION['csrf_token']; ?>" class="btn btn-danger btn-sm">сс╢сссБс</a>
            </div>
        </header>
        ...
        <section class="announcement-form animate__animated animate__fadeIn">
            ...
            ссссс╛сПсс╢ссс╝ссссс╣ссРссс╕
            </h2>
            <form method="POST" action="">
            <input type="hidden" name="action" value="create">
            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
            <div class="form-group mb-3">
                <label for="user_ids">сссс╛ссс╛ссс╗ссссс╖с</label>
                <select class="form-control" id="user_ids" name="user_ids[]" multiple required aria-label="сссс╛ссс╛ссс╗ссссс╖с">
                <?php foreach ($users as $user): ?>
                    <option value="<?php echo $user['id']; ?>"><?php echo htmlspecialchars($user['full_name']); ?></option>
                <?php endforeach; ?>
                </select>
                <small class="form-text text-muted">сс╢сссссссс╛сссс╕ссссссс сс╖ссссс╛ссс╛ссс╗ссссс╖с</small>
            </div>
            <div class="form-group mb-3">
                <label for="title">сссссс╛с</label>
                <input type="text" class="form-control" id="title" name="title" placeholder="ссссс╝ссссссс╛с" required>
            </div>
            <div class="form-group mb-3">
                <label for="date">сс╢сссс╖ссссБс</label>
                <input type="text" class="form-control" id="date" name="date" placeholder="сз. сРссссссс сбсж сс╖сРс╗сс╢ свсесжси" required>
            </div>
            <div class="form-group mb-3">
                <label for="text">свсПссРсс</label>
                <textarea class="form-control" id="text" name="text" rows="5" placeholder="ссссс╝ссвсПссРсссс╢ссс╝ссссс╣с" required></textarea>
            </div>
            <button type="submit" class="btn btn-submit">ссссасс</button>
            </form>
        </section>
        ...
        <section class="announcements animate__animated animate__fadeIn">
            <h2 class="section-title">
                ...
                сс╢ссс╝ссссс╣сссссс╢сссссасс
            </h2>
            <?php if (empty($announcements)): ?>
                <p class="text-center text-muted">сс╖ссс╢сссс╢ссс╢ссс╝ссссс╣сс</p>
            <?php else: ?>
                        ...
                        <h3 class="announcement-title">
                            ...
                            <div class="ms-auto">
                                <button class="btn btn-edit btn-sm" data-bs-toggle="modal" data-bs-target="#editModal" 
                                    ...
                                    <i class="fas fa-edit"></i> сс
                                </button>
                                <a href="?delete=<?php echo $announcement['id']; ?>&csrf_token=<?php echo $_SESSION['csrf_token']; ?>" 
                                   class="btn btn-delete btn-sm" 
                                   onclick="return confirm('сПс╛свссссссс╢сссс╢ссссс╗ссс╢ссс╝ссссс╣сссБссссссБ?')">
                                    <i class="fas fa-trash"></i> сс╗с
                                </a>
                            </div>
                        </h3>
                        <div class="announcement-user">ссссс╢сс: <?php echo htmlspecialchars($announcement['full_names'] ?: 'сссс╢ссвсссссс╜с'); ?></div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">сссс╢ссс╝ссссс╣с</h5>
                ...
                <div class="modal-body">
                    <form method="POST" action="">
                        ...
                        <div class="form-group mb-3">
                            <label for="edit_user_ids">сссс╛ссс╛ссс╗ссссс╖с</label>
                            <select class="form-control" id="edit_user_ids" name="user_ids[]" multiple required aria-label="сссс╛ссс╛ссс╗ссссс╖с">
                                <?php foreach ($users as $user): ?>
                                    <option value="<?php echo $user['id']; ?>"><?php echo htmlspecialchars($user['full_name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-group mb-3">
                            <label for="edit_title">сссссс╛с</label>
                            <input type="text" class="form-control" id="edit_title" name="title" required>
                        </div>
                        <div class="form-group mb-3">
                            <label for="edit_date">сс╢сссс╖ссссБс</label>
                            <input type="text" class="form-control" id="edit_date" name="date" required>
                        </div>
                        <div class="form-group mb-3">
                            <label for="edit_text">свсПссРсс</label>
                            <textarea class="form-control" id="edit_text" name="text" rows="5" required></textarea>
                        </div>
                        <button type="submit" class="btn btn-submit">ссссс╢сс╗с</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <nav class="bottom-nav d-lg-none">
        <a href="../homes.php" class="nav-item">
            ...
            <span>ссссРссс╛с</span>
        </a>
        <a href="#" class="nav-item">
            <i class="fas fa-calendar nav-icon"></i>
            <span>сс╢ссс╖сс╢с</span>
        </a>
        <a href="#" class="nav-item">
    ...
    <script>
        ...
        const choices = new Choices(userSelect, {
            ...
            searchEnabled: true,
            searchPlaceholderValue: 'сс╢сссссссс╗ссссс╖с...',
            noResultsText: 'сссс╖ссс╛ссс╗ссссс╖с',
            itemSelectText: '',
            maxItemCount: -1,
            classNames: {
