...
try {
    ...
    die("កំហុសក្នុងការភ្ជាប់មូលដ្ឋានទិន្នន័យ។");
}
...
if ($isAdmin && isset($_GET['username'])) {
    ...
    if (empty($targetUsername)) {
        ...
        if ($user) {
            ...
            $error = "អ្នកប្រើប្រាស់មិនត្រូវបានរកឃើញ។";
        }
    }
} elseif (!$isAdmin) {
...
if ($targetFullName && !$error) {
    try {
        ...
        $requestTypes = [
            'Annual Leave' => 'សម្រាកប្រចាំឆ្នាំ (Annual Leave)',
            'Sick Leave' => 'សម្រាកដោយជំងឺ (Sick Leave)',
            'Forgot FP' => 'ភ្លេចស្កេនមេដៃ (Forgot FP)',
            'Maternity Leave' => 'សម្រាកលំហែមាតុភាព (Maternity Leave)',
            'OT' => 'ថែមម៉ោង (OT)',
            'Early' => 'ចេញមុនម៉ោង (Early)',
            'Changing day off' => 'ប្តូរថ្ងៃសម្រាក (Changing day off)',
            ...
            foreach ($types as $type) {
                foreach ($requestTypes as $key => $definedType) {
                    if ($type === $key || $type === $definedType) {
                        if ($key === 'Annual Leave') {
                            if ($row['total_days'] == 0) {
                                $dataWarning = "ព័ត៌មានចំនួនថ្ងៃសម្រាកប្រចាំឆ្នាំមានបញ្ហា។";
                            } else {
...
<head>
    ...
    <title>វិភាគសំណើតាមប្រភេទ - ខែនេះ</title>
...
<body>
    <div class="report-container">
        <h2 class="report-title">វិភាគសំណើតាមប្រភេទ - ខែ <?php echo htmlspecialchars($selectedMonth); ?></h2>

        <form method="GET" action="<?php echo htmlspecialchars(BASE_URL); ?>" class="mb-4">
            <div class="row g-3 align-items-center justify-content-center">
                <div class="col-auto">
                    <label for="month" class="col-form-label">ជ្រើសរើសខែ:</label>
                </div>
                <div class="col-auto">
        ...
        <?php elseif ($targetFullName): ?>
            <div class="user-info">
                <strong>អ្នកប្រើ:</strong> <?php echo htmlspecialchars($targetFullName); ?>
            </div>
            <div class="leave-balance <?php echo $calculatedBalance <= 3 ? 'warning' : ''; ?>">
                <strong>សម្រាកប្រចាំឆ្នាំនៅសល់:</strong> <?php echo number_format($calculatedBalance, 2); ?> ថ្ងៃ
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-hover" role="grid" aria-describedby="requestSummaryInfo">
                    <caption id="requestSummaryInfo" class="visually-hidden">សង្ខេបសំណើខែ <?php echo htmlspecialchars($selectedMonth); ?></caption>
                    <thead>
                        <tr>
                            <th scope="col">ប្រភេទសំណើ</th>
                            <th scope="col"><?php echo $requestTypes['Annual Leave'] === 'សម្រាកប្រចាំឆ្នាំ (Annual Leave)' ? 'ចំនួនថ្ងៃ' : 'ចំនួនសំណើ'; ?></th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <?php else: ?>
                <p class="text-center">មិនមានសំណើសម្រាប់ខែនេះ។</p>
            <?php endif; ?>
        <?php else: ?>
            <p class="text-center">មិនមានទិន្នន័យអ្នកប្រើ។</p>
        <?php endif; ?>

        <div class="text-center">
            <a href="https://app.vvc.asia/requests_menu.php" class="btn-back" role="button" aria-label="ត្រឡប់ទៅម៉ឺនុយ">
        ...
        <script>
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        y: { beginAtZero: true, title: { display: true, text: 'ចំនួនសំណើ/ថ្ងៃ' } },
                        x: { title: { display: true, text: 'ប្រភេទសំណើ' } }
                    },
                    plugins: {
                        legend: { display: false },
                        title: { 
                            display: true, 
                            text: 'សំណើតាមប្រភេទ - ខែ <?php echo htmlspecialchars($selectedMonth); ?>', 
                            font: { size: 16 } 
                        },
                        tooltip: {
                            enabled: true,
                            callbacks: {
                                label: function(context) {
                                    ...
                                    return type === '<?php echo $requestTypes['Annual Leave']; ?>' ? `${label}: ${value} ថ្ងៃ` : `${label}: ${value} សំណើ`;
                                }
                            }
                        }
                    }
