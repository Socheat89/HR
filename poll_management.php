...
try {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'get_poll_results') {
        ...
        echo json_encode(['success' => false, 'error' => 'មានបញ្ហាក្នុងការភ្ជាប់ទៅ Server ។']);
        exit();
    } else {
        ...
        try {
            ...
            if ($poll_data && !empty($poll_data['results_access_key'])) { // Key is required if results_access_key is not empty
                if (!password_verify($access_key_input, $poll_data['results_access_key'])) {
                    header('Content-Type: application/json');
                    echo json_encode(['success' => false, 'error' => 'លេខសម្ងាត់មិនត្រឹមត្រូវ។']);
                    exit();
                }
            }
            ...
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'error' => 'មានបញ្ហាក្នុងការទាញយកលទ្ធផលការបោះឆ្នោត។']);
            exit();
        }
    } else {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'error' => 'លេខសម្គាល់ការបោះឆ្នោតមិនត្រឹមត្រូវ។']);
        exit();
    }
}
    ...
    if (isset($_POST['create_poll_submit'])) {
        ...
        if ($question && $quarter && $warehouse && $start_date && $end_date) {
            try {
                ...
                $message = 'ការបោះឆ្នោតថ្មីត្រូវបានបង្កើតដោយជោគជ័យ។';
                $message_type = 'success';
            } catch (PDOException $e) {
                $message = 'មានបញ្ហាក្នុងការបង្កើតការបោះឆ្នោត។';
                $message_type = 'danger';
    ...
    if (isset($_POST['edit_poll_submit'])) {
        ...
        if ($poll_id && $question && $quarter && $warehouse && $start_date && $end_date) {
            try {
                ...
                $message = 'ការបោះឆ្នោតត្រូវបានកែប្រែដោយជោគជ័យ។';
                $message_type = 'success';
            } catch (PDOException $e) {
                $message = 'មានបញ្ហាក្នុងការកែប្រែការបោះឆ្នោត។';
                $message_type = 'danger';
    ...
    if ($poll_id) {
        if ($_GET['action'] === 'toggle_active') {
            try {
                ...
                $message = 'ស្ថានភាពការបោះឆ្នោតត្រូវបានផ្លាស់ប្តូរដោយជោគជ័យ។';
                $message_type = 'success';
            } catch (PDOException $e) {
                $message = 'មានបញ្ហាក្នុងការផ្លាស់ប្តូរស្ថានភាពការបោះឆ្នោត។';
                $message_type = 'danger';
            ...
            try {
                ...
                $message = 'ការបោះឆ្នោតនិងសំឡេងឆ្នោតដែលពាក់ព័ន្ធត្រូវបានលុបដោយជោគជ័យ។';
                $message_type = 'success';
            } catch (PDOException $e) {
                $message = 'មានបញ្ហាក្នុងការលុបការបោះឆ្នោត។';
                $message_type = 'danger';
                error_log("Delete Poll Error: " . $e->getMessage());
            }
        }
    } else {
        $message = 'ID ការបោះឆ្នោតមិនត្រឹមត្រូវ។';
        $message_type = 'danger';
    }
...
try {
    ...
    $_SESSION['error'] = 'មានបញ្ហាក្នុងការទាញទិន្នន័យការបោះឆ្នោត។';
    error_log("Fetch All Polls Error: " . $e->getMessage());
    header("Location: error.php");
    exit();
}

$current_page = 'poll_management.php'; // For bottom navigation active state
?>
...
<body>
    <div class="app-container">
        ...
        <header class="app-header">
            ...
            <div class="user-profile">
                <span class="user-name" style="font-weight: 600; color: var(--dark);">
                    <?php echo htmlspecialchars($_SESSION['username'] ?? 'អ្នកគ្រប់គ្រង'); ?>
                </span>
        ...
        <h2 class="mb-4" style="font-weight: 600; color: var(--dark);">គ្រប់គ្រងការបោះឆ្នោតបុគ្គលិក</h2>
        ...
        <div class="mb-3 text-end">
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createPollModal">
                <i class="fas fa-plus-circle me-2"></i>បង្កើតការបោះឆ្នោតថ្មី
            </button>
        </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            <?php if (empty($all_polls)): ?>
                                <tr>
                                    <td colspan="8" class="text-center">មិនទាន់មានការបោះឆ្នោតណាមួយទេ។</td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="status-badge <?php echo $poll['is_active'] ? 'status-active' : 'status-inactive'; ?>">
                                                <?php echo $poll['is_active'] ? 'សកម្ម' : 'អសកម្ម'; ?>
                                            </span>
                                        </td>
                                        <td>
                                            <button class="btn btn-info btn-sm btn-action view-results-btn" data-bs-toggle="modal" data-bs-target="#viewResultsModal" data-poll-id="<?php echo $poll['id']; ?>" data-poll-question="<?php echo htmlspecialchars($poll['question']); ?>" data-key-required="<?php echo $poll['key_required'] ? 'true' : 'false'; ?>">
                                                <i class="fas fa-eye"></i> មើលលទ្ធផល
                                            ...
                                            <a href="poll_management.php?action=toggle_active&id=<?php echo $poll['id']; ?>" class="btn btn-secondary btn-sm btn-action">
                                                <i class="fas <?php echo $poll['is_active'] ? 'fa-toggle-off' : 'fa-toggle-on'; ?>"></i> <?php echo $poll['is_active'] ? 'បិទ' : 'បើក'; ?>
                                            </a>
                                            <a href="poll_management.php?action=delete&id=<?php echo $poll['id']; ?>" class="btn btn-danger btn-sm btn-action" onclick="return confirm('តើអ្នកពិតជាចង់លុបការបោះឆ្នោតនេះ និងសំឡេងឆ្នោតដែលពាក់ព័ន្ធទាំងអស់មែនទេ?');">
        ...
        <div class="modal fade" id="createPollModal" tabindex="-1" aria-labelledby="createPollModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form action="poll_management.php" method="POST">
                        <div class="modal-header">
                            <h5 class="modal-title" id="createPollModalLabel">បង្កើតការបោះឆ្នោតថ្មី</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="create_question" class="form-label">សំណួរ</label>
                                <input type="text" class="form-control" id="create_question" name="question" required>
                            </div>
                            <div class="mb-3">
                                <label for="create_quarter" class="form-label">ត្រីមាសទី</label>
                                <select class="form-control" id="create_quarter" name="quarter" required>
                                    <option value="">-- ជ្រើសរើស --</option>
                                    <option value="១">ត្រីមាសទី ១</option>
                                    <option value="២">ត្រីមាសទី ២</option>
                                    <option value="៣">ត្រីមាសទី ៣</option>
                                    <option value="៤">ត្រីមាសទី ៤</option>
                                </select>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="create_start_date" class="form-label">កាលបរិច្ឆេទចាប់ផ្តើម</label>
                                <input type="datetime-local" class="form-control" id="create_start_date" name="start_date" required>
                            </div>
                            <div class="mb-3">
                                <label for="create_end_date" class="form-label">កាលបរិច្ឆេទបញ្ចប់</label>
                                <input type="datetime-local" class="form-control" id="create_end_date" name="end_date" required>
                            </div>
                            <div class="mb-3">
                                <label for="create_results_access_key" class="form-label">លេខសម្ងាត់សម្រាប់មើលលទ្ធផល (ទុកទទេបើមិនត្រូវការ)</label>
                            ...
                            <div class="form-check mb-3">
                                ...
                                <label class="form-check-label" for="create_restrict_users">
                                    កំណត់អ្នកប្រើប្រាស់ដែលអនុញ្ញាតបោះឆ្នោត
                                </label>
                            </div>
                            <div class="mb-3" id="create_usernames_div" style="display: none;">
                                <label class="form-label">ជ្រើសរើសអ្នកប្រើប្រាស់ដែលអនុញ្ញាត</label>
                                <div class="border p-3" style="max-height: 200px; overflow-y: auto;" id="create_users_list">
                                    <!-- Users will be populated by JS -->
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">អ្នកប្រើប្រាស់ដែលលើកលែងពេលបើក (optional)</label>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" name="create_poll_submit" class="btn btn-primary">បង្កើត</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="editPollModal" tabindex="-1" aria-labelledby="editPollModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form action="poll_management.php" method="POST">
                        <div class="modal-header">
                            <h5 class="modal-title" id="editPollModalLabel">កែប្រែការបោះឆ្នោត</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <input type="hidden" name="poll_id" id="edit_poll_id">
                            <div class="mb-3">
                                <label for="edit_question" class="form-label">សំណួរ</label>
                                <input type="text" class="form-control" id="edit_question" name="question" required>
                            </div>
                            <div class="mb-3">
                                <label for="edit_quarter" class="form-label">ត្រីមាសទី</label>
                            ...
                            <div class="mb-3">
                                <label for="edit_start_date" class="form-label">កាលបរិច្ឆេទចាប់ផ្តើម</label>
                                <input type="datetime-local" class="form-control" id="edit_start_date" name="start_date" required>
                            </div>
                            <div class="mb-3">
                                <label for="edit_end_date" class="form-label">កាលបរិច្ឆេទបញ្ចប់</label>
                                <input type="datetime-local" class="form-control" id="edit_end_date" name="end_date" required>
                            </div>
                            <div class="mb-3">
                                <label for="edit_results_access_key" class="form-label">លេខសម្ងាត់សម្រាប់មើលលទ្ធផល (ទុកទទេដើម្បីមិនផ្លាស់ប្តូរ)</label>
                                <input type="password" class="form-control" id="edit_results_access_key" name="results_access_key" autocomplete="off">
                                <small class="form-text text-muted">បញ្ចូលលេខសម្ងាត់ថ្មីដើម្បីផ្លាស់ប្តូរ ឬទុកទទេដើម្បីរក្សាលេខសម្ងាត់ដែលមានស្រាប់។</small>
                            </div>
                            <div class="form-check mb-3">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">បោះបង់</button>
                            <button type="submit" name="edit_poll_submit" class="btn btn-primary">រក្សាទុកការកែប្រែ</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="viewResultsModal" tabindex="-1" aria-labelledby="viewResultsModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="viewResultsModalLabel">លទ្ធផលការបោះឆ្នោត: <span id="resultsPollQuestion"></span></h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div id="accessKeyForm" class="mb-3">
                            <p id="keyRequiredMessage" class="alert alert-warning" style="display: none;"><i class="fas fa-lock me-2"></i>សូមបញ្ចូលលេខសម្ងាត់ដើម្បីមើលលទ្ធផលនៃការបោះឆ្នោតនេះ។</p>
                            <div class="input-group">
                                <input type="password" class="form-control" id="resultsAccessKeyInput" placeholder="លេខសម្ងាត់" autocomplete="off">
                                <button class="btn btn-primary" type="button" id="submitAccessKeyBtn">បង្ហាញលទ្ធផល</button>
                            </div>
                            <div id="keyErrorMessage" class="text-danger mt-2" style="display: none;"></div>
                        </div>

                        <div id="resultsContentArea" style="display: none;">
                            <p class="mb-3">ចំនួនសំឡេងឆ្នោតសរុប: <strong id="resultsTotalVotes"></strong></p>
                            ...
                            <div class="individual-votes-section">
                                <h5 class="mb-3">សំឡេងឆ្នោតលម្អិត:</h5>
                            ...
                            <div id="certificatePrintSection" class="mt-4 text-end" style="display: none;">
                                <a id="printCertificateBtn" href="#" target="_blank" class="btn btn-success">
                                    <i class="fas fa-award"></i> បោះពុម្ព Certificate
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editPollModal.addEventListener('show.bs.modal', function (event) {
                ...
                modalTitle.textContent = 'កែប្រែការបោះឆ្នោត: ' + question;
            ...
            submitAccessKeyBtn.addEventListener('click', function() {
                ...
                if (isKeyRequired && accessKey === '') {
                    keyErrorMessage.textContent = 'សូមបញ្ចូលលេខសម្ងាត់។';
            ...
            async function fetchAndDisplayResults(pollId, accessKey) {
                individualVotesContent.innerHTML = '<p class="text-center text-muted">កំពុងទាញយកទិន្នន័យ...</p>';
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        if (aggregatedResults.length === 0) {
                            pollResultsContent.innerHTML = '<p class="text-center">មិនទាន់មានសំឡេងឆ្នោតសម្រាប់ការបោះឆ្នោតនេះទេ។</p>';
                        } else {
                        ...
                        if (individualVotes.length === 0) {
                            individualVotesContent.innerHTML = '<p class="text-center">មិនទាន់មានសំឡេងឆ្នោតលម្អិតសម្រាប់ការបោះឆ្នោតនេះទេ។</p>';
                        } else {
                        ...
                        keyErrorMessage.textContent = data.error || 'មានបញ្ហាក្នុងការទាញយកលទ្ធផល។';
                        keyErrorMessage.style.display = 'block';
                    ...
                    resultsContentArea.style.display = 'none'; // Hide results
                    keyRequiredMessage.style.display = 'none';
                    keyErrorMessage.textContent = 'មានបញ្ហាក្នុងការភ្ជាប់ទៅ Server ។';
                    keyErrorMessage.style.display = 'block';
                    individualVotesContent.innerHTML = ''; // Clear loading message
                    pollResultsContent.innerHTML = '';
