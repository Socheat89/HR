...
$IG_ACCESS_TOKEN = 'EAAb4Bh6lZCg4BPa2lz9mLioZAUqgcmABCiRT8ILmpNO0IP5St6IqhAe8GoQchGdtvOm4PF8GIKidALKuUBxKFsyjZBkx4otUvRWNZC2BEhTYW6Mqmj3gtjVLUHOXSu6huZAUwsxzwu4BJZCQ8ZAYY3G3zWs4eD0LbHZCF5bbI9wPgwjhkAI0FFypTFMRVDI9nuyJq0Y7ZAHNrYAZDZD'; // Long-lived access token
$IG_USER_ID = '17841476699831237'; // Business account ID
        ...
        if (isset($_FILES['media']) && !empty($_FILES['media']['name'][0])) {
            foreach ($_FILES['media']['tmp_name'] as $key => $tmp_name) {
                if ($_FILES['media']['error'][$key] === UPLOAD_ERR_OK) {
                    $file_path = $tmp_name;
                    $original_file_name = basename($_FILES['media']['name'][$key]); // Keep original name for logging
                    $file_size = $_FILES['media']['size'][$key];
                    $mime_type = mime_content_type($file_path);
                    
                    if ($file_size > $max_file_size) continue; // Skip if too large

                    // === FIX START: Sanitize the filename to make it URL-safe ===
                    // This prevents errors when Instagram's server tries to access the media URL.
                    // It replaces spaces and any characters that are not letters, numbers, dots, underscores, or hyphens with a single underscore.
                    $safe_file_name = preg_replace('/[^a-zA-Z0-9\._-]/', '_', $original_file_name);
                    $new_file_path = $upload_dir . uniqid() . '_' . $safe_file_name;
                    // === FIX END ===
                    ...
                    if (move_uploaded_file($file_path, $new_file_path)) {
                        ...
                        $media_files[] = [
                            'path' => $new_file_path,
                            'url' => 'https://app.vvc.asia/manager-post/' . $new_file_path, // This URL is now safe
                            'is_image' => strpos($mime_type, 'image/') === 0,
                            'name' => $original_file_name // Use original name for display/logging
                        ];
                    }
                }
            }
...
/**
 * Waits for an Instagram media container to be ready for publishing.
 * Polls the container status until it's 'FINISHED', 'EXPIRED', or 'ERRORED'.
 *
 * @param string $container_id The ID of the media container.
 * @param string $access_token The user's access token.
 * @return bool True if the container is FINISHED, false otherwise.
 */
function waitForMediaToBeReady($container_id, $access_token) {
    $max_attempts = 20; // Try for a maximum of 20 times (2 minutes)
    $wait_time = 6;     // Wait 6 seconds between each attempt

    for ($i = 0; $i < $max_attempts; $i++) {
        $url = "https://graph.facebook.com/v20.0/{$container_id}?fields=status_code&access_token={$access_token}";
        ...
        error_log("Polling IG Container {$container_id}: Status is {$status_code}");

        if ($status_code === 'FINISHED') {
            return true; // Ready to be published!
        }

        if ($status_code === 'EXPIRED' || $status_code === 'ERRORED') {
            error_log("IG Container {$container_id} failed with status: {$status_code}");
            return false; // Failed, no point in waiting.
        }

        // If not finished, wait before checking again
        sleep($wait_time);
    }
    
    error_log("IG Container {$container_id} timed out after {$max_attempts} attempts.");
    return false; // Timed out
}

function postToInstagram($media_files, $caption, $access_token, $user_id) {
    try {
        if (count($media_files) > 1) { // Carousel Post
            $children = [];
            foreach ($media_files as $file) {
                ...
                if (isset($create_json['id'])) {
                    ...
                    // If it's a video, we must wait for it to finish processing
                    if (!$file['is_image']) {
                        if (waitForMediaToBeReady($container_id, $access_token)) {
                            $children[] = $container_id;
                        } else {
                            error_log("Carousel video item failed processing: {$container_id}");
                            echo "Carousel video item failed processing for {$file['name']}<br>";
                        }
                    } else {
            ...
            if (isset($create_json['id'])) {
                ...
                $is_ready_to_publish = true; // Assume ready by default for images
                // If it's a video, wait for it to be ready
                if (!$file['is_image']) {
                    $is_ready_to_publish = waitForMediaToBeReady($container_id, $access_token);
                }

                if ($is_ready_to_publish) {
                    // This part only runs if the media is ready (image or finished video)
                    $publish_url = "https://graph.facebook.com/v20.0/$user_id/media_publish";
                    $publish_data = [
                        'access_token' => $access_token,
                    ...
                    error_log("Instagram single video failed processing for {$file['name']}: {$container_id}");
                    echo "Instagram single video failed processing for {$file['name']}: {$container_id}<br>";
                }
