...
<head>
  ...
  <title>ទីតាំងអតិថិជនទាំងអស់</title>
...
<body class="overflow-hidden bg-gray-100">
  ...
  <div class="relative h-screen w-screen">
    ...
    <div class="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 bg-white p-2 rounded-lg shadow-lg w-11/12 max-w-md">
      <input type="text" id="searchBox" class="flex-1 px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="ស្វែងរកអតិថិជន..." />
    ...
    <div id="listPanel" class="fixed bottom-0 left-0 right-0 z-10 bg-white shadow-lg rounded-t-lg flex flex-col h-40 md:absolute md:top-24 md:left-4 md:bottom-auto md:right-auto md:w-full md:max-w-sm md:h-auto md:max-h-[calc(100vh-8rem)] md:rounded-lg">
        ...
        <h2 class="text-xl font-bold text-gray-800 mb-2 flex-shrink-0 px-4 pt-2">បញ្ជីអតិថិជន</h2>
  ...
  <div id="addLocationModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-2xl w-11/12 max-w-md">
      <h3 class="text-2xl font-bold mb-4">បន្ថែមទីតាំងថ្មី</h3>
      ...
      <form id="addLocationForm">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div><label for="latitude" class="block text-sm font-semibold mb-1">រយៈទទឹង (Latitude)</label><input type="text" id="latitude" name="latitude" class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100" readonly required></div>
          <div><label for="longitude" class="block text-sm font-semibold mb-1">រយៈបណ្ដោយ (Longitude)</label><input type="text" id="longitude" name="longitude" class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100" readonly required></div>
        </div>
        <div class="mb-6"><label for="customerName" class="block text-sm font-semibold mb-1">ឈ្មោះអតិថិជន</label><input type="text" id="customerName" name="name" class="w-full px-3 py-2 border border-gray-300 rounded-md" required></div>
        <div class="flex justify-end gap-4"><button type="button" id="cancelAddBtn" class="px-5 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">បោះបង់</button><button type="submit" class="px-5 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">រក្សាទុក</button></div>
      </form>
    </div>
  </div>
  ...
  <div id="routingModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-2xl w-11/12 max-w-md">
      <h3 class="text-2xl font-bold mb-4">ចាប់ផ្តើមការធ្វើដំណើរ</h3>
      <p id="routing-modal-message" class="mb-4 text-center font-semibold"></p>
      <form id="routingForm">
        <div class="mb-6"><label for="endPoint" class="block text-sm font-semibold mb-1">ទៅកាន់ទីតាំង (To)</label><select id="endPoint" class="w-full px-3 py-2 border border-gray-300 rounded-md" required></select></div>
        <div class="flex justify-end gap-4"><button type="button" id="cancelRoutingBtn" class="px-5 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">បោះបង់</button><button type="submit" class="px-5 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">ចាប់ផ្តើម</button></div>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    function handleFindRoute(e) {
        e.preventDefault();
        routingModalMessageEl.textContent = 'កំពុងស្នើសុំទីតាំងបច្ចុប្បន្ន...';
        routingModalMessageEl.className = 'mb-4 text-center font-semibold text-blue-600';

        navigator.geolocation.getCurrentPosition(
            (position) => {
                routingModalMessageEl.textContent = 'កំពុងបង្កើតការធ្វើដំណើរ...';
                ...
                fetch('start_trip.php', { method: 'POST', body: formData })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        // CORRECTED LINE 1
                        routingModalMessageEl.textContent = 'ការធ្វើដំណើរបានចាប់ផ្តើម! កំពុងបើកផែនទីនាំផ្លូវ...';
                        ...
                        routingModalMessageEl.textContent = data.message || 'មានបញ្ហាក្នុងការបង្កើតការធ្វើដំណើរ។';
                        routingModalMessageEl.className = 'mb-4 text-center font-semibold text-red-600';
                    }
                })
                .catch(error => {
                    // CORRECTED LINE 2
                    routingModalMessageEl.textContent = 'មានបញ្ហាបណ្តាញ។';
                    routingModalMessageEl.className = 'mb-4 text-center font-semibold text-red-600';
                });
            },
            (error) => {
                // CORRECTED LINE 3
                routingModalMessageEl.textContent = "មិនអាចកំណត់ទីតាំងបានទេ។ សូមអនុញ្ញាតឱ្យកម្មវិធីរុករក (Browser) ប្រើប្រាស់ទីតាំងរបស់អ្នក។";
                routingModalMessageEl.className = 'mb-4 text-center font-semibold text-red-600';
            }, 
    ...
    getLocationBtn.addEventListener('click', () => {
        ...
        modalMessageEl.textContent = 'កំពុង​ស្នើសុំ​ទីតាំង...'; modalMessageEl.className = 'mb-4 text-center font-semibold text-blue-600';
        navigator.geolocation.getCurrentPosition( (position) => {
                document.getElementById('latitude').value = position.coords.latitude.toFixed(8); 
                document.getElementById('longitude').value = position.coords.longitude.toFixed(8);
                modalMessageEl.textContent = 'ចាប់​បាន​ទីតាំង​ដោយ​ជោគជ័យ!'; modalMessageEl.className = 'mb-4 text-center font-semibold text-green-600';
