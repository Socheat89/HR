    ...
    <div class="report-container">
        ...
        <form method="GET" class="filter-form">
            <select name="status">
                ...
                <option value="pending" <?php echo $filter_status === 'pending' ? 'selected' : ''; ?>>·····∂·</option>
                <option value="approved" <?php echo $filter_status === 'approved' ? 'selected' : ''; ?>>··∂··¢··ª··ê·è</option>
                <option value="rejected" <?php echo $filter_status === 'rejected' ? 'selected' : ''; ?>>··∂····∑··Å·</option>
            </select>
            <input type="date" name="request_date" value="<?php echo htmlspecialchars($filter_date); ?>">
            <input type="text" name="request_type" placeholder="·····Å·····æ" value="<?php echo htmlspecialchars($filter_type); ?>">
            <select name="sort_by">
                <option value="id" <?php echo $sort_by === 'id' ? 'selected' : ''; ?>>··Å·Å·····∂··</option>
                <option value="request_type" <?php echo $sort_by === 'request_type' ? 'selected' : ''; ?>>·····Å·</option>
                <option value="request_date" <?php echo $sort_by === 'request_date' ? 'selected' : ''; ?>>·ê·······æ··ª·</option>
                <option value="created_at" <?php echo $sort_by === 'created_at' ? 'selected' : ''; ?>>··∂····∑····Å······æ·è</option>
                <option value="status" <?php echo $sort_by === 'status' ? 'selected' : ''; ?>>···ê·∂···∂·</option>
            </select>
            <select name="sort_order">
                <option value="DESC" <?php echo $sort_order === 'DESC' ? 'selected' : ''; ?>>··ª·</option>
                <option value="ASC" <?php echo $sort_order === 'ASC' ? 'selected' : ''; ?>>·°·æ·</option>
            </select>
            <button type="submit" class="btn btn-primary"><i class="fas fa-filter me-2"></i>·è····</button>
            <button type="button" class="btn btn-secondary" onclick="window.location.href='../requests/submit_request.php'"><i class="fas fa-plus me-2"></i>····æ··ª··ê···∏</button>
        </form>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php if (empty($requests)): ?>
                        <tr>
                            <td colspan="11" class="text-center">··∑···∂·····æ··∂··Ω··è···º···∂·····æ···Å·</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($requests as $request): ?>
                            <tr>
                                ...
                                <td class="status-<?php echo htmlspecialchars($request['status']); ?>">
                                    ...
                                    echo $status === 'pending' ? '·····∂·' : ($status === 'approved' ? '··∂··¢··ª··ê·è' : '··∂····∑··Å·');
                                    ?>
                                </td>
                                ...
                                <td>
                                    <?php if ($request['status'] === 'pending'): ?>
                                        <form method="POST" style="display: inline;">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-success"><i class="fas fa-check"></i> ·¢··ª··ê·è</button>
                                        </form>
                                        <form method="POST" style="display: inline;">
                                            <input type="hidden" name="request_id" value="<?php echo $request['id']; ?>">
                                            <input type="hidden" name="action" value="reject">
                                            <button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-times"></i> ···∑··Å·</button>
                                        </form>
                                    <?php endif; ?>
                                    <a href="delete_request.php?id=<?php echo $request['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('·è·æ·¢·······∂····Å·ê·∂·····ª·····æ··Å·?');"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <div class="text-center mt-4">
            <button type="button" class="btn btn-secondary" onclick="window.location.href='https://app.vvc.asia/homes.php'">
                <i class="fas fa-arrow-left me-2"></i>·è···°·······
            </button>
        </div>
    </div>
