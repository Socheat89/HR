...
try {
    ...
    $khmer_names = ['department' => 'ផ្នែក', 'position' => 'មុខតំណែង', 'branch' => 'សាខា'];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            switch ($action) {
                ...
                case 'add':
                    ...
                    if (empty($type) || empty($name) || !in_array($type, $category_types)) {
                        throw new Exception("សូមបំពេញប្រភេទ និងឈ្មោះឱ្យបានត្រឹមត្រូវ។");
                    }

                    $stmt_check = $pdo->prepare("SELECT COUNT(*) FROM categories WHERE type = ? AND name = ?");
                    $stmt_check->execute([$type, $name]);
                    if ($stmt_check->fetchColumn() > 0) {
                        throw new Exception("ឈ្មោះ **" . htmlspecialchars($name) . "** មាននៅក្នុងផ្នែក **" . htmlspecialchars($type) . "** រួចហើយ។");
                    }

                    $stmt = $pdo->prepare("INSERT INTO categories (type, name) VALUES (?, ?)");
                    $stmt->execute([$type, $name]);
                    $_SESSION['success'] = "បានបញ្ចូល **" . htmlspecialchars($name) . "** ទៅក្នុងផ្នែក **" . htmlspecialchars($khmer_names[$type]) . "** ដោយជោគជ័យ! ✅";
                    break;
                ...
                case 'edit':
                    ...
                    if ($id <= 0 || empty($newName) || empty($newType) || !in_array($newType, $category_types)) {
                           throw new Exception("ទិន្នន័យសម្រាប់កែប្រែមិនពេញលេញ ឬមិនត្រឹមត្រូវ។");
                    }

                    $stmt_check = $pdo->prepare("SELECT id FROM categories WHERE type = ? AND name = ? AND id != ?");
                    $stmt_check->execute([$newType, $newName, $id]);
                    if ($stmt_check->fetch()) {
                        throw new Exception("ឈ្មោះថ្មី **" . htmlspecialchars($newName) . "** មាននៅក្នុងប្រភេទ **" . htmlspecialchars($khmer_names[$newType]) . "** រួចហើយ។");
                    }

                    $stmt = $pdo->prepare("UPDATE categories SET type = ?, name = ? WHERE id = ?");
                    $stmt->execute([$newType, $newName, $id]);
                    $_SESSION['success'] = "បានកែប្រែទិន្នន័យ (ID: $id) ទៅជា **" . htmlspecialchars($newName) . "** ដោយជោគជ័យ! ✏️";
                    break;

                case 'delete':
                    $id = (int)($_POST['category_id'] ?? 0);
                    if ($id <= 0) {
                        throw new Exception("លេខសម្គាល់មិនត្រឹមត្រូវសម្រាប់លុប។");
                    }

                    $stmt = $pdo->prepare("DELETE FROM categories WHERE id = ?");
                    $stmt->execute([$id]);
                    $_SESSION['success'] = "បានលុបទិន្នន័យដោយជោគជ័យ! 🗑️";
                    break;
            ...
            if ($e->getCode() == 23000) { // SQLSTATE for integrity constraint violation
                $_SESSION['error'] = "កំហុសមូលដ្ឋានទិន្នន័យ: ទិន្នន័យនេះមានរួចហើយ ឬមានបញ្ហាជាមួយ Relationship។";
            } else {
    ...
    $error_message = "កំហុសក្នុងការតភ្ជាប់ ឬដំណើរការមូលដ្ឋានទិន្នន័យ: " . $e->getMessage();
...
<head>
    ...
    <style>
        ...
        :root {
            --primary-bg: #161b22;       /* ផ្ទៃខាងក្រោយចម្បង - ខ្មៅស្រាល (from settings_control) */
            --secondary-bg: #0d1117;       /* ផ្ទៃខាងក្រោយទីពីរ - ខ្មៅដិត (from settings_control) */
            --card-bg: rgba(22, 27, 34, 0.9); /* ផ្ទៃខាងក្រោយកាត (មានតម្លាភាពបន្តិច) (from settings_control) */
            --border-color: rgba(255, 255, 255, 0.1); /* ពណ៌គែម - ស្តើង (from settings_control) */
            --accent-color: #ffd700;       /* ពណ៌សង្កត់ចម្បង - មាសសុទ្ធ (from settings_control) */
            --accent-hover: #ffea70;       /* ពណ៌សង្កត់ពេល Hover (from settings_control) */
            --text-primary: #f0f6fc;       /* ពណ៌អក្សរចម្បង - សភ្លឺ (from settings_control) */
            --text-secondary: #ffffff;      /* ពណ៌អក្សរទីពីរ - ប្រផេះស្រាល (from settings_control) */
            --success: #2ea043;          /* ពណ៌ជោគជ័យ - បៃតង (from settings_control) */
            --danger: #da3633;          /* ពណ៌គ្រោះថ្នាក់ - ក្រហម (from settings_control) */
       ...
       /* --- Sidebar Navigation (របាររុករកចំហៀង) --- */
        aside {
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight text-accent-hover drop-shadow-sm">
                             <i class="fas fa-cogs mr-2 text-accent-color"></i> ការកំណត់ទាំងអស់។
                        </h1>
                        <p class="text-text-secondary mt-2 text-lg font-medium italic">
                            Telegram Bot, ផ្នែក, មុខតំណែង, សាខា
                        </p>
                    </div>
                </div>
        ...
        <div class="main-content-wrapper">
            ...
            <ul class="nav nav-tabs" id="settingsTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="telegram-tab" data-bs-toggle="tab" data-bs-target="#telegramSettings" type="button" role="tab" aria-controls="telegramSettings" aria-selected="true">
                        <i class="fab fa-telegram-plane me-2"></i>ការកំណត់ Telegram Bot
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="categories-tab" data-bs-toggle="tab" data-bs-target="#categoriesControl" type="button" role="tab" aria-controls="categoriesControl" aria-selected="false">
                        <i class="fas fa-sitemap me-2"></i>ផ្នែក, មុខតំណែង, សាខា
                    </button>
                </li>
            </ul>
            ...
            <div class="tab-content" id="settingsTabsContent">
                ...
                <div class="tab-pane fade show active" id="telegramSettings" role="tabpanel" aria-labelledby="telegram-tab">
                    ...
                    <div class="telegram-settings-card animate__animated animate__fadeInUp">
                        ...
                        <form method="POST" action="settings_control.php">
                            ...
                            <button type="submit" class="btn-base btn-primary w-100"><i class="fas fa-save me-2"></i> រក្សាទុក Bot Token</button>
                        </form>
                    </div>

                    <!-- Existing Telegram Groups Block -->
                    <div class="telegram-settings-card animate__animated animate__fadeInUp">
                        <h4 class="telegram-section-title"><i class="fas fa-users-cog me-2"></i> ក្រុម Telegram ដែលមានស្រាប់</h4>
                                ...
                                <div class="telegram-group-item mb-4" id="group-<?php echo htmlspecialchars($group['group_id']); ?>">
                                    ...
                                    <form method="POST" action="settings_control.php" class="mb-4">
                                        ...
                                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-2 mt-4 pt-3 border-top border-telegram-border-color">
                                            <button type="submit" class="btn-base btn-primary flex-grow-1"><i class="fas fa-save me-2"></i> រក្សាទុកការផ្លាស់ប្ដូរក្រុម</button>
                                            <button type="button" class="btn-base btn-danger flex-grow-1" onclick="confirmDeleteGroup('<?php echo htmlspecialchars($group['group_id']); ?>')"><i class="fas fa-trash-alt me-2"></i> លុបក្រុម</button>
                                        </div>
                                    </form>
                                    ...
                                    <div class="sub-block mt-4 pt-3">
                                        ...
                                        <?php if (empty($group['thread_ids'])): ?>
                                            <p class="text-text-secondary text-center py-2 opacity-75">មិនទាន់មានតួនាទីត្រូវបានកំណត់សម្រាប់ក្រុមនេះនៅឡើយទេ។</p>
                                        <?php else: ?>
                                                            ...
                                                            <tr>
                                                                <form method="POST" action="settings_control.php" class="d-inline">
                                                                    ...
                                                                    <td class="d-flex align-items-center justify-content-evenly">
                                                                        <button type="submit" class="btn-base btn-primary btn-sm me-1" title="រក្សាទុក Thread">
                                                                            <i class="fas fa-save"></i>
                                                                        </button>
                                                                </form>
                                                                <form method="POST" action="settings_control.php" class="d-inline" onsubmit="return confirm('តើអ្នកពិតជាចង់លុបតួនាទីនេះមែនទេ?');">
                                                                    <input type="hidden" name="action" value="delete_thread">
                                                                    <input type="hidden" name="thread_map_id" value="<?php echo htmlspecialchars($thread_item['thread_map_id']); ?>">
                                                                        <button type="submit" class="btn-base btn-danger btn-sm" title="លុប Thread">
                                        ...
                                        <form method="POST" action="settings_control.php">
                                            ...
                                            <div class="row g-2 align-items-end">
                                                ...
                                                <div class="col-md-2">
                                                    <button type="submit" class="btn-base btn-success btn-sm w-100"><i class="fas fa-plus me-2"></i> បន្ថែម</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div> <!-- End sub-block -->
                    ...
                    <div class="telegram-settings-card animate__animated animate__fadeInUp">
                        <h4 class="telegram-section-title"><i class="fas fa-plus-square me-2"></i> បន្ថែមក្រុម Telegram ថ្មី</h4>
                        <form method="POST" action="settings_control.php">
                            ...
                            <button type="submit" class="btn-base btn-primary w-100"><i class="fas fa-plus-circle me-2"></i> បន្ថែមក្រុម</button>
                        </form>
                    </div>
                ...
                <div class="tab-pane fade" id="categoriesControl" role="tabpanel" aria-labelledby="categories-tab">
                    <div class="card-base mb-8 shadow-lg">
                        <div class="mb-4">
                            <h5 class="text-xl font-bold text-accent-hover"><i class="fas fa-plus-circle me-2"></i>បន្ថែមជម្រើសថ្មី</h5>
                        </div>
                        <form method="POST" action="settings_control.php" class="row g-3">
                            <input type="hidden" name="action" value="add">
                            <div class="col-md-4 mb-3">
                                <label for="category_type" class="form-label">ជ្រើសរើសប្រភេទ:</label>
                                <select id="category_type" name="category_type" class="form-select" required>
                                    <option value="" class="bg-secondary">-- ជ្រើសរើស --</option>
                            ...
                            <div class="col-md-5 mb-3">
                                <label for="category_name" class="form-label">ឈ្មោះជម្រើស (ខ្មែរ/អង់គ្លេស):</label>
                                <input type="text" id="category_name" name="category_name" class="form-control" placeholder="បញ្ចូលឈ្មោះថ្មី" required>
                            </div>
                            <div class="col-md-3 d-flex align-items-end">
                                <button type="submit" class="btn-base btn-primary w-full"><i class="fas fa-save me-1"></i>រក្សាទុក</button>
                            </div>
                        </form>
                    </div>
                                        ...
                                        <?php foreach ($dynamic_categories[$type] as $category): ?>
                                            <li class="category-item"
                                                ...
                                                <div class="item-actions">
                                                    <button type="button" class="edit-btn" title="កែប្រែ" onclick="openEditModal(this)">
                                                        <i class="fas fa-edit"></i>
                                                    </button>

                                                    <form method="POST" action="settings_control.php" onsubmit="return confirm('តើអ្នកពិតជាចង់លុបជម្រើស «<?php echo htmlspecialchars($category['name']); ?>» នេះមែនទេ?');" class="d-inline">
                                                        <input type="hidden" name="action" value="delete">
                                                        <input type="hidden" name="category_id" value="<?php echo (int)$category['id']; ?>">
                                                        <button type="submit" class="delete-btn" title="លុប"><i class="fas fa-times-circle"></i></button>
                                                    </form>
                                                </div>
                                            </li>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <li class="p-3 text-text-secondary text-center opacity-75">មិនទាន់មានជម្រើសទេ។</li>
                                    <?php endif; ?>
                                </ul>
                            </div>
                        </div>
            ...
            <div class="text-center mt-4 hidden sm:hidden md:block">
                <a href="dashboard.php" class="btn-base btn-primary">
                    <i class="fas fa-arrow-left me-2"></i>ត្រឡប់ទៅផ្ទាំងគ្រប់គ្រង
                </a>
            </div>
        ...
        <div class="modal-content">
          <form method="POST" action="settings_control.php">
            ...
            <div class="modal-header">
              <h5 class="modal-title" id="editModalLabel"><i class="fas fa-edit me-2"></i>កែប្រែជម្រើស</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <div class="mb-3">
                <label for="edit-category-type" class="form-label">ប្រភេទ (Category Type)</label>
              ...
              <div class="mb-3">
                <label for="edit-category-name" class="form-label">ឈ្មោះថ្មី (New Name)</label>
                <input type="text" id="edit-category-name" name="new_category_name" class="form-control" required>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn-base bg-secondary text-text-secondary" data-bs-dismiss="modal">បោះបង់</button>
              <button type="submit" class="btn-base btn-primary"><i class="fas fa-save me-1"></i>រក្សាទុកការកែប្រែ</button>
            </div>
          </form>
        </div>
      </div>
