...
// =============================================================================
$main_db_table = 'ks2_consolidated_staff';
$new_staff_db_table = 'ks2_new_staff';
...
function handle_post_request(PDO $pdo, string $main_db_table, string $new_staff_db_table): string
{
    ...
    if (isset($_POST['save_new_staff'])) {
        ...
        if (json_last_error() !== JSON_ERROR_NONE)
            return "បរាជ័យ: ទិន្នន័យបុគ្គលិកថ្មីមិនត្រឹមត្រូវ។";
...
<head>
    ...
    <title>របាយការណ៍ - ផ្សារកួរស្រូវ២</title>
    ...
    <style>
        ...
        tbody th.shift-header {
            background-color: #e9ecef;
            font-weight: bold;
        }

        tbody th.gender-header {
...
<body>
    <div class="container">
        ...
        <?php if ($message): ?>
            <div
                class="message <?= strpos($message, 'បរាជ័យ') !== false || strpos($message, 'កំហុស') !== false ? 'error' : 'success' ?>">
                <?= htmlspecialchars($message) ?>
            </div><?php endif; ?>

        <div id="capture-area">
            <h1 class="text-h1">របាយការណ៍វត្តមានបុគ្គលិក - ផ្សារកួរស្រូវ២</h1>
                ...
                <thead>
                    <tr>
                        ...
                        <?php foreach ($department_configs as $config): ?>
                            <th><?= htmlspecialchars($config['label']) ?></th><?php endforeach; ?>
                        <th>សរុបរួម</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($daily_record): ?>
                        <tr>
                            ...
                            <?php foreach ($department_configs as $key => $config): ?>
                                <td class="editable" data-column="<?= $key ?>_female_morning">
                                    <?= $daily_record["{$key}_female_morning"] ?? 0 ?>
                                </td>
                            <?php endforeach; ?>
                        ...
                        <tr>
                            ...
                            <?php foreach ($department_configs as $key => $config): ?>
                                <td class="editable" data-column="<?= $key ?>_male_morning">
                                    <?= $daily_record["{$key}_male_morning"] ?? 0 ?>
                                </td>
                            <?php endforeach; ?>
                        ...
                        <tr>
                            ...
                            <th class="gender-header">ស្រី</th>
                            <?php foreach ($department_configs as $key => $config): ?>
                                <td class="editable" data-column="<?= $key ?>_female_evening">
                                    <?= $daily_record["{$key}_female_evening"] ?? 0 ?>
                                </td>
                            <?php endforeach; ?>
                        ...
                        <tr>
                            <th class="gender-header">ប្រុស</th>
                            <?php foreach ($department_configs as $key => $config): ?>
                                <td class="editable" data-column="<?= $key ?>_male_evening">
                                    <?= $daily_record["{$key}_male_evening"] ?? 0 ?>
                                </td>
                            <?php endforeach; ?>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="<?= count($department_configs) + 3 ?>">មិនមានទិន្នន័យសម្រាប់ថ្ងៃនេះទេ។</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
                <?php if ($daily_record): ?>
                    ...
                    <?php if (!empty($new_staff_records)):
                        foreach ($new_staff_records as $row): ?>
                            <tr data-id="<?= $row['id'] ?>">
                                ...
                                <td class="editable" data-column="note" style="white-space: pre-wrap;">
                                    <?= htmlspecialchars($row['note']) ?>
                                </td>
                                <td class="editable" data-column="reports_date"><?= htmlspecialchars($row['reports_date']) ?>
                                </td>
                        ...
                        <tr class="no-data-row">
                            <td colspan="6">មិនមានទិន្នន័យសម្រាប់ថ្ងៃនេះទេ។</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
